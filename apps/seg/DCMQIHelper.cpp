...
std::string DCMQIHelper::FloatToStrScientific(float f) {
    std::ostringstream sstream;
    sstream << std::scientific << f;
    return sstream.str();
}

void DCMQIHelper::checkValidityOfFirstSrcImage(DcmSegmentation *segdoc) {
    FGInterface &fgInterface = segdoc->getFunctionalGroups();
    bool isPerFrame = false;
    FGDerivationImage *derimgfg =
            OFstatic_cast(FGDerivationImage*, fgInterface.get(0, DcmFGTypes::EFG_DERIVATIONIMAGE, isPerFrame));
    assert(derimgfg);
    assert(isPerFrame);
...
void DCMQIHelper::TokenizeString(std::string str, std::vector<std::string> &tokens, std::string delimiter){
...
void DCMQIHelper::SplitString(std::string str, std::string &head, std::string &tail, std::string delimiter){
    // http://stackoverflow.com/questions/14265581/parse-split-a-string-in-c-using-string-delimiter-standard-c
...
float* DCMQIHelper::getCIEXYZFromRGB(unsigned *rgb, float *cieXYZ){
 ...
 **/
unsigned* DCMQIHelper::getRGBFromCIEXYZ(float* cieXYZ, unsigned *rgb) {
...
**/
float* DCMQIHelper::getCIELabFromCIEXYZ(float *cieXYZ, float *cieLab){
...
/**
 <p>Convert CIE 1976 L*, a*, b* to CIEXYZ.</p>
  ...
  return     array of length 3 containing X,Y,Z values
 **/
float* DCMQIHelper::getCIEXYZFromCIELab(float *cieLab, float *cieXYZ) {
 ...
 **/
unsigned* DCMQIHelper::getIntegerScaledCIELabFromCIELab(float *cieLab, unsigned *cieLabScaled){
...
/**
      ...
      **/
float* DCMQIHelper::getCIELabFromIntegerScaledCIELab(unsigned* cieLabScaled, float* cieLab){
...
CodeSequenceMacro DCMQIHelper::StringToCodeSequenceMacro(std::string str){
...
DSRCodedEntryValue DCMQIHelper::StringToDSRCodedEntryValue(std::string str){
    std::string tail, code, designator, meaning;
    SplitString(str, code, tail, ",");
    SplitString(tail, designator, meaning, ",");
